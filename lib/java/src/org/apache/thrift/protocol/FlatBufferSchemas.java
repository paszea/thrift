...
public class FlatBufferSchemas {
  ...
  public static void main(String[] args) throws Exception {
    FlatBufferSchemas schemas = new FlatBufferSchemas("/Users/xunliu/workplace/flatbuffers/t.bfbs");
    com.google.flatbuffers.reflection.Object table = schemas.getTableSchema("flatb_np.TestObj");
    Field e = schemas.getFieldSchema(table, "e");

    System.out.println("table name=" + table.name() + " field.name="
            + e.name());
    Field mld = schemas.getFieldSchemaByThriftId(table, 20);
    System.out.println("mld.name=" + mld.name());

    Field lbin = schemas.getFieldSchema(table, "lbin");
    System.out.println("lbin isObjectType? " + schemas.isObjectType(lbin));
    System.out.println("lbin isVectorType? " + schemas.isVectorType(lbin));
    System.out.println("lbin isAuxVectorType? " + schemas.isAuxVectorType(lbin));

    table = schemas.getFieldTableSchema(e);
    System.out.println("e.type=" + table.name());
    table = schemas.getFieldElemTableSchema(lbin);
    System.out.println("lbin.type=" + table.name());
  }
};
